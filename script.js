window.onload = function(){
    ...
    function showQuestion(quetionNumber){
    	document.querySelector('.question').innerHTML = quiz[step]['q'] + ' :';
    	let answer = '';
    	for(let key in quiz[step]['a']){
    		answer += `<li data-v='${key}' class="answer__variant">${quiz[step]['a'][key]}</li>`;
    	}
    ...
    document.onclick = function(event){
    	...
    	if(event.target.classList.contains('answer__variant') && step < quiz.length){
